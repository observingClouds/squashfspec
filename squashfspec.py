...
class SquashFSFileSystem(AbstractFileSystem):
    ...
    def __init__(self, fo=None, offset=0, **kwargs):
        ...
        if fo is None:
             raise ValueError("SquashFSFileSystem requires 'fo' (file-like object or path)")
    ...
    def _strip_protocol(cls, path):
        ...
        # If the protocol is still there, it's often because fsspec's split_protocol
        # didn't match it or it's nested.
        ...
        if "://" in path:
            # Handle cases like some_path://inner_path (should not happen for us, but for safety)
            path = path.split("://", 1)[1]
